...
bool shieldy_sdk::is_file_exists(const std::string &name) {
...
std::vector<unsigned char> shieldy_sdk::read_native_library_bytes() {
...
bool shieldy_sdk::verify_native_library() {
    ...
    HRESULT hr = win_utils::rsa_verify(BCRYPT_SHA256_ALGORITHM, get_rsa_key().c_str(), signature.data(),
                                       nativeSha256.data(),
                                       nativeSha256.size());
    if (hr != S_OK) {
        _com_error err(hr);
...
void shieldy_sdk::update_if_available() {
...
void shieldy_sdk::xor_bytes(char *data, size_t dataSize, const unsigned char *key, size_t keySize) {
